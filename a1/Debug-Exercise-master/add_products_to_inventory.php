...
for ($x=0; $x<=7; $x++) { 
	...
	if (!empty($_POST['productname' . $x]))  {
		...
		//added in the = '' to make sure it was empty
		$produceItem = '';
		...
		switch ($thisProduceType) {
			...
			case "v": //veg
				...
				$produceItem = new veggie($x, $thisProductName, $thisProductPriceType, $thisProductPrice); //got rid of the _ in the $thisProductName because that was not the proper way, and it wouldn't work
				$typeType = "Veggie";
				break;
		}
		
		if($produceItem->price_type == 2) {
			$pType = "Each"; //I added the ; at the end of the each statment to make it execute. 
		}
		else {
			$pType = "Per Lb";
		}
